...
export default function Studio() {
  const caps: [string, string][] = [
    ['design', 'identity · art direction · brand systems'],
    ['code', 'production web · full-stack · github-native · you own everything'],
    ['systems', 'the studio runs on its own tooling — content engines, custom workflows, automation built in-house'],
  ];
  const stickers = ['cannabis', 'tech', 'collectibles', 'lifestyle', 'food'];

  return (
    <section id="studio" className="section dark-section">
      ...
      <div className="container" style={{ position: 'relative', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 320px), 1fr))', gap: 'clamp(40px, 6vw, 80px)', alignItems: 'center' }}>
        <div>
          <div className="section-tag" style={{ marginBottom: 22 }}>
            ...
            <span className="hand" style={{ color: 'var(--butter)', fontSize: 22, transform: 'rotate(-2deg)' }}>the studio ✿</span>
          </div>
          ...
          <p style={{ fontFamily: 'var(--font-body)', fontSize: 'clamp(15px, 1.3vw, 17px)', lineHeight: 1.65, color: 'rgba(255,250,240,0.72)', margin: '28px 0 0', maxWidth: 480 }}>
            run by vanessa saddi — <span style={{ color: 'var(--paper)' }}>thirteen years of craft, one studio, all layers.</span>
          </p>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 10, marginTop: 30, maxWidth: 480 }}>
            {caps.map(([k, v]) => (
              <div key={k} style={{ padding: '14px 16px', border: '1px solid rgba(255,250,240,0.14)', borderRadius: 12, background: 'rgba(255,250,240,0.04)' }}>
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--paper)', fontWeight: 500, letterSpacing: '0.08em' }}>{k}</div>
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'rgba(255,250,240,0.55)', marginTop: 4, lineHeight: 1.5 }}>{v}</div>
              </div>
            ))}
          </div>
